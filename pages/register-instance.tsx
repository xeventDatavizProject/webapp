...
const RegisterInstance: NextPage = () => {
  ...
  const onSubmit = async (data: instanceTypeSchema) => {
    await dispatch(createInstance(data)).then(res => {
      ...
      console.log(res);
      status === 'fulfilled' ? (router.push('/dashboard'), setErrorMessage('')) : setErrorMessage('Error on create instance');
    });
  };
        ...
        <div className='max-w-xl mx-auto text-center'>
          ...
          <section className='bg-white text-black-primary rounded-2xl py-8 px-12 mt-10'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='mt-6'>
              <div className='mb-6'>
                <Controller
                  ...
                  render={({ field, fieldState: { error } }) => (
                    <Input label='User' placeholder='Enter your database username' type='text' {...field} error={error?.message} />
                  )}
                />
              </div>
              <div className='mb-8'>
                <Controller
                  ...
                  defaultValue=''
                  render={({ field, fieldState: { error } }) => (
                    <Input label='Hostname' placeholder='Enter the hostname' type='text' {...field} error={error?.message} />
                  )}
                />
              </div>
              <div className='mb-8'>
                <Controller
                  ...
                  defaultValue=''
                  render={({ field, fieldState: { error } }) => (
                    <Input label='Port' placeholder='Enter your database port' type='text' {...field} error={error?.message} />
                  )}
                />
              </div>
              <div className='mb-8'>
                <Controller
                  ...
                  defaultValue=''
                  render={({ field, fieldState: { error } }) => (
                    <Input label='Password' placeholder='Enter your database password' type='password' {...field} error={error?.message} />
                  )}
                />
              </div>
              ...
              <div>{errorMessage && <Paragraph className='text-error-primary mt-4'>{errorMessage}</Paragraph>}</div>
            </form>
          </section>
