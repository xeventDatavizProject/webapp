...
const LoginModal: FC<LoginModalProps> = ({ show, onClose, onClickForgot }) => {
  ...
  return (
    ...
    >
      ...
      <Dialog.Panel className='relative m-auto z-[99] bg-[#e3e7ec] text-black w-full rounded-xl md:rounded-2xl px-6 py-8 md:p-12 xl:p-20 max-w-xl'>
        <h5 className='text-center text-2xl mb-4'>Connect to your account</h5>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className='space-y-4 max-w-md mx-auto p-4 md:p-12 bg-blue-quaternary rounded-xl md:rounded-2xl'>
            <Controller
              ...
              name='email'
              defaultValue=''
              render={({ field, fieldState: { error } }) => <Input label='Email' type='email' {...field} error={error?.message} />}
            />
            <Controller
              control={control}
              name='password'
